    ...
    fun generate(): JSONObject {
        ...
        return JSONObject().apply {
            ...
            put("runtime", JSONObject().apply {
                ...
                put("privacy_classifier_version", PrivacyClassifier.CLASSIFIER_VERSION)
                put("privacy_repair_applied_version", PrivacyRepair.appliedVersion(context))
            })
            put("scheduler", JSONObject().apply {
                put("pending_now", GuardianScheduler.isScheduled(context))
                put("dev_period_minutes", GuardianScheduler.periodMinutes())
                put("schedule_attempt_count", schedulerState.scheduleAttemptCount())
                put("recovery_count", schedulerState.recoveryCount())
                put("job_run_count", schedulerState.jobRunCount())
                put("job_stop_count", schedulerState.jobStopCount())
                put("last_schedule_attempt_at", schedulerState.lastScheduleAttemptMs().takeIf { it > 0 }?.let(::iso) ?: JSONObject.NULL)
                put("last_schedule_reason", schedulerState.lastScheduleReason() ?: JSONObject.NULL)
                put("last_schedule_result", schedulerState.lastScheduleResult())
                put("last_schedule_pending", schedulerState.lastSchedulePending())
                put("last_scheduler_check_at", schedulerState.lastSchedulerCheckMs().takeIf { it > 0 }?.let(::iso) ?: JSONObject.NULL)
                put("last_scheduler_check_reason", schedulerState.lastSchedulerCheckReason() ?: JSONObject.NULL)
                put("last_scheduler_check_pending", schedulerState.lastSchedulerCheckPending())
                put("last_job_start_at", schedulerState.lastJobStartMs().takeIf { it > 0 }?.let(::iso) ?: JSONObject.NULL)
                put("last_job_finish_at", schedulerState.lastJobFinishMs().takeIf { it > 0 }?.let(::iso) ?: JSONObject.NULL)
                put("last_job_stop_at", schedulerState.lastJobStopMs().takeIf { it > 0 }?.let(::iso) ?: JSONObject.NULL)
            })
